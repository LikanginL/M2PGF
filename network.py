...
class Land_Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(align_output.size(1)):
            ...
            t_align_attention_feat = torch.softmax(t_align_attention_feat, 1)
            t_align_attention_feat = t_align_attention_feat.view(t_align_attention_feat_ori.size(0), 1,t_align_attention_feat_ori.size(1),t_align_attention_feat_ori.size(2))
            if start:
...
class MineLoss(nn.Module):
    ...
    def forward(self, pred_xy, pred_x_y):
        return  - np.log2(np.exp(1)) * (torch.mean(pred_xy) - torch.log(torch.mean(torch.exp(pred_x_y))))
        #return torch.mean(pred_xy) - torch.log(torch.mean(torch.exp(pred_x_y)))
...
class DiffLoss(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        #diff_loss = torch.sum((input1_l2 * input2_l2).sum(dim=1).pow(2))
        return diff_loss

network_dict = {'Feat_Enc':Feat_Enc, 'Land_Detect':Land_Detect, 'AU_Detect':AU_Detect, 'Land_Disc':Land_Disc,
                'Texture_Enc':Texture_Enc, 'Generator':Generator, 'Discriminator':Discriminator,'DiffLoss':DiffLoss,'MineLoss':MineLoss}